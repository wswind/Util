...
namespace Util.Biz.Payments.Extensions {
    ...
    public static class Extensions {
        ...
        public static void AddWechatpayEx( this IServiceCollection services,IConfiguration configuration)
        {
            if (configuration is null)
            {
                throw new ArgumentNullException(nameof(configuration));
            }
            services.Configure<CustomWechatPayConfig>(configuration.GetSection("WechatPayConfig"));
            services.TryAddTransient<WechatpayConfig>(sp =>
            {
                var cwpc = sp.GetRequiredService<IOptions<CustomWechatPayConfig>>().Value;
                var certpath = System.IO.Path.Combine(System.AppContext.BaseDirectory, cwpc.CertificatePath);
                WechatpayConfig wpc = new WechatpayConfig()
                {
                    AppId = cwpc.AppId,
                    MerchantId = cwpc.MerchantId,
                    PrivateKey = cwpc.PrivateKey,
                    NotifyUrl = cwpc.NotifyUrl,
                    Certificate = certpath,
                    CertificatePassword = cwpc.CertificatePassword
                };
                return wpc;
            });

            services.TryAddScoped<IWechatpayConfigProvider>(sp =>
            {
                var config = sp.GetRequiredService<WechatpayConfig>();
                return WechatpayConfigProvider.CreateWechatpayConfigProvider(config);
            });
